...
const COMMAND_KEYWORD_INDEX = Object.freeze(
  Object.entries(COMMAND_INTENT_META)
    .map(([command, meta]) => {
      ...
      // merge all patterns into a single regex with alternation to reduce .test() calls.
      // tokens are sorted by length to prevent shorter phrases matching inside longer ones.
      const source = tokens
        .map((t) => {
...
export function findKeywordCommand(text: string): string | null {
  // use standard for loop to avoid iterator overhead in hot path.
...
export function parseCommandIntent(text: string): { command: string; args: string } | null {
  ...
  if (direct?.[1]) {
    ...
    // optimize: O(1) command lookup using COMMAND_INTENT_META or alias index.
