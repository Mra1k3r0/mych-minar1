...
export function sanitizeUrl(url: string): string {
  ...
  try {
    ...
    let out = url.replace(/^(?:\w+:)?\/\/([^/]+@)/, (match: string, authority: string): string => {
      const atIndex = authority.lastIndexOf("@");
      if (atIndex === -1) return match;
      return match.replace(authority.slice(0, atIndex), "[redacted]:[redacted]");
    });
